...
$showDetails = ShowsHandlers::getShowTmDB($showID)['data'] ?? [];

function body($showDetails): string
{
    $title = $showDetails['name'] ?? $movieDetails['original_name'] ?? null;
    $overview = $showDetails['overview'] ?? null;
    $image = $showDetails['poster_path'] ?? $movieDetails['backdrop_path'] ?? null;
    $vote = $showDetails['vote_count'] ?? 0;
    $date =  (new DateTime($showDetails['first_air_date']))->format("F d, Y");
    $byName = $showDetails['production_companies'][0]['name'] ?? null;
...
function tags($showDetails): string{
...
function otherInfo($showDetails): string
{
    $duration = $showDetails['episode_run_time'][0] ?? 0;
    $vote_average = $showDetails['vote_average'] ?? 0.0;
    $pop = $showDetails['popularity'] ?? 0;
    $oriLang = $showDetails['spoken_languages'][0]['english_name'] ?? null;
    return <<<EOD
...
function productionCompanies($showDetails): string
{
    $companies = $showDetails['production_companies'] ?? [];
    $list = null;
...
function form($showDetails): string{

    if(Globals::method() === "POST" && !empty(Globals::post("addmovietmdb")))
    {
        if((new  \groups\GroupShows())->saveShows(Globals::get("show-id"))){
 ...
 <form method="POST" action="$uri" class="search-form">
                                <div class="input-group"> 
                                    <div class="input-group-append">
                                        <button name="addmovietmdb" value="addmtm" type="submit" class="btn btn-primary submit-btn">Create Show</button>
                                    </div>
                                </div>
                            </form>
EOD;
...
<div class="main-wrapper">
    ...
    <div class="page-wrapper">
        <div class="content">
            <div class="row">
                <div class="col-sm-12">
                    <h4 class="page-title">Show View</h4>
                </div>
            </div>
            <div class="row">
                <div class="col-md-8">
                    <div class="blog-view"><?php echo body($showDetails); ?>
                ...
                <aside class="col-md-4">
                    <div class="widget search-widget">
                        <h5>Show Form</h5><?php echo form($showDetails); ?>
                    </div>
                    <?php echo productionCompanies($showDetails); ?>
                    <?php echo otherInfo($showDetails); ?>
                    <?php echo tags($showDetails); ?>
                </aside>
            </div>
        </div>
